...
const jadibot = async (reply, client, id) => {
	...
	conn.browserDescription = ['Kanappi Bot', '', '3.0']
	conn.on('qr', async qr => {
		let bot = await qrcode.toDataURL(qr, {
			scale: 8
		})
		let buffer = new Buffer.from(bot.replace('data:image/png;base64,', ''), 'base64')
		bot = await client.sendMessage(id, buffer, MessageType.image, {
			caption: '*Scan this QR to become a temporary bot!*\n1. *Click the three dots in the top right corner*\n2. *Tap WhatsApp Web*\n3. *Scan this QR*\n\n*QR Expires in 30 seconds*'
		})
		setTimeout(() => {
	...
	conn.on('chat-update', async (message) => {
		require('../Kanappi.js')(conn, message)
	})
}
